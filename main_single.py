...
def test(checkpoint_path, class_num, round, args):
    for key in building.keys():
        ...
        old_feature_map, adj_lists, old_query = collectGraph_test(test_dataset[key]['feature_path'], node_num, args.feat_dim, args.num_sample, args.suffix, round)
        ...
        if args.use_cuda:
            query = query.cuda()
        new_query = graphsage.encoder.queryForward(query)
